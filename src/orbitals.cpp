...
//		IO routines
long OrbitalRec::GetSize(BufferFile *Buffer) {
...
long OrbitalRec::WriteToBuffer(BufferFile *Buffer) {
	...
	total += Buffer->Write((Ptr) &NumAlphaOrbs, sizeof(long));
	total += Buffer->Write((Ptr) &NumBetaOrbs, sizeof(long));
	total += Buffer->Write((Ptr) &BaseWavefunction, sizeof(long));
	total += Buffer->Write((Ptr) &OrbitalType, sizeof(long));
	if (Vectors) {
		code = 2;
		total += Buffer->Write((Ptr) &code, sizeof(long));
		length = NumBasisFunctions*NumAlphaOrbs*sizeof(float);
		total += Buffer->Write((Ptr) &length, sizeof(long));
		total += Buffer->Write((Ptr) Vectors, length);
	}
	if (VectorsB) {
		code = 3;
		total += Buffer->Write((Ptr) &code, sizeof(long));
		length = NumBasisFunctions*NumBetaOrbs*sizeof(float);
		total += Buffer->Write((Ptr) &length, sizeof(long));
		total += Buffer->Write((Ptr) VectorsB, length);
	}
	if (Energy) {
		code = 4;
		total += Buffer->Write((Ptr) &code, sizeof(long));
		length = NumAlphaOrbs*sizeof(float);
		total += Buffer->Write((Ptr) &length, sizeof(long));
		total += Buffer->Write((Ptr) Energy, length);
	}
	if (EnergyB) {
		code = 5;
		total += Buffer->Write((Ptr) &code, sizeof(long));
		length = NumBetaOrbs*sizeof(float);
		total += Buffer->Write((Ptr) &length, sizeof(long));
		total += Buffer->Write((Ptr) EnergyB, length);
	}
	if (OrbOccupation) {
		code = 6;
		total += Buffer->Write((Ptr) &code, sizeof(long));
		length = NumAlphaOrbs*sizeof(float);
		total += Buffer->Write((Ptr) &length, sizeof(long));
		total += Buffer->Write((Ptr) OrbOccupation, length);
	}
	if (OrbOccupationB) {
		code = 7;
		total += Buffer->Write((Ptr) &code, sizeof(long));
		length = NumBetaOrbs*sizeof(float);
		total += Buffer->Write((Ptr) &length, sizeof(long));
		total += Buffer->Write((Ptr) OrbOccupationB, length);
	}
	if (SymType) {
		code = 8;
		total += Buffer->Write((Ptr) &code, sizeof(long));
		length = NumAlphaOrbs*5*sizeof(char);
		total += Buffer->Write((Ptr) &length, sizeof(long));
		total += Buffer->Write((Ptr) SymType, length);
	}
	if (SymTypeB) {
		code = 9;
		total += Buffer->Write((Ptr) &code, sizeof(long));
		length = NumBetaOrbs*5*sizeof(char);
		total += Buffer->Write((Ptr) &length, sizeof(long));
		total += Buffer->Write((Ptr) SymTypeB, length);
	}
	if (Label) {
		code = 10;
		total += Buffer->Write((Ptr) &code, sizeof(long));
	...
	if (NumOccupiedAlphaOrbs > 0) {
		code = 13;
		total += Buffer->Write((Ptr) &code, sizeof(long));
		length = sizeof(long);
		total += Buffer->Write((Ptr) &length, sizeof(long));
		total += Buffer->Write((Ptr) &NumOccupiedAlphaOrbs, length);
	}
	if (NumOccupiedBetaOrbs > 0) {
		code = 14;
		total += Buffer->Write((Ptr) &code, sizeof(long));
		length = sizeof(long);
		total += Buffer->Write((Ptr) &length, sizeof(long));
		total += Buffer->Write((Ptr) &NumOccupiedBetaOrbs, length);
	}
...
OrbitalRec * OrbitalRec::Read(BufferFile *Buffer, long NumBasisFuncs, long ByteCount) {
	long	code, length, total;

	total = Buffer->Read((Ptr) &code, sizeof(long));
	if ((code != 1)&&(code != 11)&&(code != 12)) throw DataError();
	total += Buffer->Read((Ptr) &length, sizeof(long));
	OrbitalRec * temp = new OrbitalRec(Buffer, code, length);
	if (temp == NULL) throw MemoryError();
	total += length;
	temp->NumBasisFunctions = NumBasisFuncs;
	temp->ReadCodes(Buffer, NumBasisFuncs, ByteCount-total);
	return temp;
}
void OrbitalRec::ReadCodes(BufferFile * Buffer, long NumBasisFuncs, long ByteCount) {
	...
	while (total<ByteCount) {
		...
		switch (code) {
			case 2:
				Vectors = new float[NumAlphaOrbs*NumBasisFuncs];
				if (!Vectors) throw MemoryError();
				if (length != (NumAlphaOrbs*NumBasisFuncs*sizeof(float)))
					throw DataError();
				Buffer->Read((Ptr) Vectors, length);
			break;
			case 3:
				VectorsB = new float[NumBetaOrbs*NumBasisFuncs];
				if (!VectorsB) throw MemoryError();
				if (length != (NumBetaOrbs*NumBasisFuncs*sizeof(float)))
					throw DataError();
				Buffer->Read((Ptr) VectorsB, length);
			break;
			case 4:
				Energy = new float[NumAlphaOrbs];
				if (!Energy) throw MemoryError();
				if (length != NumAlphaOrbs*sizeof(float)) throw DataError();
				Buffer->Read((Ptr) Energy, length);
			break;
			case 5:
				EnergyB = new float[NumBetaOrbs];
				if (!EnergyB) throw MemoryError();
				if (length != NumBetaOrbs*sizeof(float)) throw DataError();
				Buffer->Read((Ptr) EnergyB, length);
			break;
			case 6:
				OrbOccupation = new float[NumAlphaOrbs];
				if (!OrbOccupation) throw MemoryError();
				if (length != NumAlphaOrbs*sizeof(float)) throw DataError();
				Buffer->Read((Ptr) OrbOccupation, length);
			break;
			case 7:
				OrbOccupationB = new float[NumBetaOrbs];
				if (!OrbOccupationB) throw MemoryError();
				if (length != NumBetaOrbs*sizeof(float)) throw DataError();
				Buffer->Read((Ptr) OrbOccupationB, length);
			break;
			case 8:
				SymType = new char[NumAlphaOrbs*5];
				if (!SymType) throw MemoryError();
				if (length != 5*NumAlphaOrbs*sizeof(char)) throw DataError();
				Buffer->Read((Ptr) SymType, length);
			break;
			case 9:
				SymTypeB = new char[NumBetaOrbs*5];
				if (!SymTypeB) throw MemoryError();
				if (length != 5*NumBetaOrbs*sizeof(char)) throw DataError();
				Buffer->Read((Ptr) SymTypeB, length);
			break;
			case 10:
				Label = new char[length];
				if (!Label) throw MemoryError();
				Buffer->Read((Ptr) Label, length);
			break;
			case 13:
				Buffer->Read((Ptr) &NumOccupiedAlphaOrbs, length);
			break;
			case 14:
				Buffer->Read((Ptr) &NumOccupiedBetaOrbs, length);
			break;
