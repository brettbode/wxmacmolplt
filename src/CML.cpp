...
long MolDisplayWin::WriteCMLFile(BufferFile * Buffer, bool allFrames, bool AllData) {
    XMLSetup();
    ...
    if (MainData->Description != NULL)
        xmlRoot->addAttribute(CML_convert(titleAttr), MainData->Description);
    if (AllData) {  //write out non-CML base items
        ...
        //      InputOptions, DrawMode/DrawLabels flags, Additional window related info.
        {
            ...
            for (int i=0; i<4; i++) {
                for (int j=0; j<4; j++) {
                    if ((i>0)||(j>0)) rotbuf << " ";
                    rotbuf << MainData->TotalRotation[j][i]; 
                }
            }
        ...
        if (QD3DActive) {
            XMLElement * Ele = MetaDataListXML->addChildElement(CML_convert(MetaDataElement));
            Ele->addAttribute(CML_convert(nameAttr), CML_convert(MMP_Use3DMode));
            Ele->addAttribute(CML_convert(contentAttr), "true");
        }
    }
    if (allFrames) {
        ...
        MainData->cFrame->WriteCMLFrame(xmlRoot, AllData);
    }
...
void MolDisplayWin::WriteWindowData(XMLElement * parent) const {
    Rect tRect;
    GetWindowRect(&tRect);
    char line[kMaxLineLength];
    XMLElement * t = parent->addChildElement(CML_convert(MMP_MainWinRect));
    snprintf(line, kMaxLineLength, "%d", tRect.left);
    t->addAttribute(kLeftXML, line);
    snprintf(line, kMaxLineLength, "%d", tRect.right);
    t->addAttribute(kRightXML, line);
    snprintf(line, kMaxLineLength, "%d", tRect.top);
    t->addAttribute(kTopXML, line);
    snprintf(line, kMaxLineLength, "%d", tRect.bottom);
    t->addAttribute(kBottomXML, line);

    if (Coords) {
        Coords->GetWindowRect(&tRect);
        t = parent->addChildElement(CML_convert(MMP_CoordWinRect));
        snprintf(line, kMaxLineLength, "%d", tRect.left);
        t->addAttribute(kLeftXML, line);
        snprintf(line, kMaxLineLength, "%d", tRect.right);
        t->addAttribute(kRightXML, line);
        ...
        snprintf(line, kMaxLineLength, "%d", tRect.bottom);
        t->addAttribute(kBottomXML, line);
        t->addAttribute(kVisibleXML, trueXML);
    }
    if (BondsDlog) {
        BondsDlog->GetWindowRect(&tRect);
        t = parent->addChildElement(CML_convert(MMP_BondsWinRect));
        snprintf(line, kMaxLineLength, "%d", tRect.left);
        t->addAttribute(kLeftXML, line);
        snprintf(line, kMaxLineLength, "%d", tRect.right);
        t->addAttribute(kRightXML, line);
    ...
    if (EPlotWin) {
        EPlotWin->GetWindowRect(&tRect);
        t = parent->addChildElement(CML_convert(MMP_EPlotWinRect));
        snprintf(line, kMaxLineLength, "%d", tRect.left);
        t->addAttribute(kLeftXML, line);
        snprintf(line, kMaxLineLength, "%d", tRect.right);
        t->addAttribute(kRightXML, line);
    ...
    if (FreqPlotWin) {
        FreqPlotWin->GetWindowRect(&tRect);
        t = parent->addChildElement(CML_convert(MMP_FreqWinRect));
        snprintf(line, kMaxLineLength, "%d", tRect.left);
        t->addAttribute(kLeftXML, line);
        snprintf(line, kMaxLineLength, "%d", tRect.right);
        t->addAttribute(kRightXML, line);
    ...
    if (SurfaceDlog) {
        SurfaceDlog->GetWindowRect(&tRect);
        t = parent->addChildElement(CML_convert(MMP_SurfaceWinRect));
        snprintf(line, kMaxLineLength, "%d", tRect.left);
        t->addAttribute(kLeftXML, line);
        snprintf(line, kMaxLineLength, "%d", tRect.right);
        t->addAttribute(kRightXML, line);
    ...
    if (PrefsDlog) {
        PrefsDlog->GetWindowRect(&tRect);
        t = parent->addChildElement(CML_convert(MMP_PrefsWinRect));
        snprintf(line, kMaxLineLength, "%d", tRect.left);
        t->addAttribute(kLeftXML, line);
        snprintf(line, kMaxLineLength, "%d", tRect.right);
        t->addAttribute(kRightXML, line);
...
long MolDisplayWin::OpenCMLFile(BufferFile * Buffer) {
    short errors = 0;
    long result = 0;
        ...
        if (xDoc->parse()) {
            ...
            if (CML_convert(root->getName(), rootname)) {
                switch (rootname) {
                    case CMLElement:
                    {
                        ...
                        for (int i=0; i<children->length(); i++) {
                            ...
                            switch (elName) {
                                        ...
                                        if (!strcmp(title, CML_convert(MMP_RotationMatrix))) {
                                            ...
                                            if ((cols==4)&&(rows==4)) {
                                                ...
                                                if (val) {
                                                    ...
                                                    for (int i=0; i<4; i++) {
                                                        for (int j=0; j<4; j++) {
                                                            sscanf(&(val[pos]),"%f%n", &(MainData->TotalRotation[j][i]), &nchar);
                                                            pos += nchar;
                                                        }
                                                    }
                                                }
                                ...
                                {
                                    XMLElementList * mdchildren = child->getChildren();
                                    if (mdchildren) {
                                        if (mdchildren->length()>0) {
                                            for (int j=0; j<mdchildren->length(); j++) {
                                                XMLElement * mdchild = mdchildren->item(j);
                                                        ...
                                                        switch (attr) {
                                                            case MMP_MaxSize:
                                                            {
                                                                ...
                                                                if (mdchild->getAttributeValue(CML_convert(contentAttr), val)) {
                                                                    if (val > 0.0) MainData->MaxSize = val;
                                                                }
                                                            }
                                                                break;
                                                            case MMP_WindowSize:
                                                            {
                                                                float val;
                                                                if (mdchild->getAttributeValue(CML_convert(contentAttr), val)) {
                                                                    if (val > 0.0) MainData->WindowSize = val;
                                                                }
                                                            }
                                                                break;
                                                            ...
                                                            case MMP_DrawSpecialAtoms:
                                                            {
                                                                bool val;
                                                                if (mdchild->getAttributeValue(CML_convert(contentAttr), val))
                                                                    MainData->SetSpecialAtomDrawMode(val);
                                                            }
                                                                break;
                                                            case MMP_ShowAxis:
                                                            {
                                                            ...
                                                            case MMP_ShowAtomLabels:
                                                            {
                                                                bool val;
                                                                if (mdchild->getAttributeValue(CML_convert(contentAttr), val))
                                                                    MainData->SetAtomLabelDrawMode(val);
                                                            }
                                                                break;
                                                            ...
                                                            case MMP_ShowAtomNumbers:
                                                            {
                                                                bool val;
                                                                if (mdchild->getAttributeValue(CML_convert(contentAttr), val))
                                                                    MainData->SetAtomNumbersDrawMode(val);
                                                            }
                                                                break;
                                                            ...
                                                            {   //There should be only one
                                                                if (!(MainData->IntCoords)) {
                                                                    ...
                                                                    MainData->IntCoords->ReadXML(mdchild);
                                                                }
                                                            }
                                                                break;
                                                            case MMP_BasisSet:
                                                                if (!MainData->Basis) MainData->Basis = BasisSet::ReadXML(mdchild);
                                                                break;
                                                            case MMP_InputOptions:
                                                            {
                                                                if (MainData->InputOptions) delete MainData->InputOptions;
                                                                MainData->InputOptions = new InputData;
                                                                MainData->InputOptions->ReadXML(mdchild);
                                                            }
                                                                break;
                    ...
                    break;
                    default:
                        MessageAlert("\pUnknown root element.");
                        errors++;
                }
            } else {
                MessageAlert("\pUnable to parse XML.");
                errors++;
            }
        } else {
            errors++;
            MessageAlert("\pAn error occured parsing cml file");
        }
        delete xDoc;
    }
    catch (...) {
    ...
    XMLShutdown();
    if (errors > 0){
//      MessageAlert("\pA xerces error occured parseing cml file");
        AbortOpen(0);
    }
    if (result) {
...
#ifdef UseOpenGL
        if (gOpenGLAvailable) {
            if (QD3DActive) OpenGLInitWindow();
        }
#endif
        ResetModel(false);
    }
...
void MolDisplayWin::ReadWindowData(XMLElement * parent) {
    XMLElementList * children = parent->getChildren();
    if (children) {
        if (children->length()>0) {
            for (int i=0; i<children->length(); i++) {
                XMLElement * child = children->item(i);
                MMP_MetaWindowItems itype;
                long temp, test;
                Rect tRect;
                if (CML_convert(child->getName(), itype)) {
                    switch (itype) {
                        case MMP_MainWinRect:
                            test = 0;
                            if (child->getAttributeValue(kLeftXML, temp)) {
                                tRect.left = temp;
                                test ++;
                            }
                            if (child->getAttributeValue(kRightXML, temp)) {
                                tRect.right = temp;
                                test ++;
                            }
                            if (child->getAttributeValue(kTopXML, temp)) {
                                tRect.top = temp;
                                test ++;
                            }
                            if (child->getAttributeValue(kBottomXML, temp)) {
                                tRect.bottom = temp;
                                test ++;
                            }
                            if (test == 4) {
